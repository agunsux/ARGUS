...
class TransactionChallengeService {
  ...
  static async createChallenge({
    ...
    confirmingActorId = null,
    expectedActorRole,
    expectedActorId
  }) {
    const role = issuingActorRole || expectedActorRole;
    const actorId = issuingActorId || expectedActorId;

    if (!orderId || !eventId || !actionType || !role || !actorId) {
      const err = new Error('Missing required challenge parameters (orderId, eventId, actionType, issuingActorRole, issuingActorId)');
      err.code = 'INVALID_CHALLENGE_PARAMS';
      throw err;
    }

    // MANDATORY TRUST INVARIANT: PIC CANNOT GENERATE ENTRY_CONFIRMED CHALLENGE!
    if (actionType === 'ENTRY_CONFIRMED' && role === 'pic') {
    ...
    // MANDATORY TRUST INVARIANT: PIC CANNOT GENERATE HANDOFF_READY CHALLENGE!
    if (actionType === 'HANDOFF_READY' && role === 'pic') {
      const err = new Error('PIC cannot generate HANDOFF_READY challenge. The handoff challenge must be generated by the Seller.');
      err.code = 'PIC_CANNOT_ISSUE_HANDOFF_CHALLENGE';
      throw err;
    }

    // For ENTRY_CONFIRMED, issuer MUST be buyer
    if (actionType === 'ENTRY_CONFIRMED' && role !== 'buyer') {
      const err = new Error('Only Buyer can request entry confirmation challenge');
      err.code = 'INVALID_CHALLENGE_ISSUER';
      throw err;
    }

    // For HANDOFF_READY, issuer MUST be seller
    if (actionType === 'HANDOFF_READY' && role !== 'seller') {
      const err = new Error('Only Seller can request handoff challenge');
      err.code = 'INVALID_CHALLENGE_ISSUER';
      throw err;
    }
    ...
    const challenge = {
      ...
      issuing_actor_role: role,
      issuing_actor_id: actorId,
      confirming_actor_role: confirmingActorRole,
      confirming_actor_id: confirmingActorId,
      expected_actor_role: role,
  ...
  static async verifyAndConsumeChallenge({ orderId, actionType, providedCode, consumingActorId, consumingActorRole }) {
    ...
    // Verify consumer role: Both HANDOFF and ENTRY must be confirmed by PIC
    if (consumingActorRole && consumingActorRole !== 'pic') {
      const err = new Error(`Role mismatch: Expected 'pic' to confirm challenge, got '${consumingActorRole}'`);
      err.code = 'CONSUMER_ROLE_MISMATCH';
      err.status = 403;
      throw err;
    }

    // PIC cannot confirm a challenge issued by PIC
    if (challenge.issuing_actor_role === 'pic') {
      const err = new Error('PIC cannot confirm a challenge issued by PIC');
      err.code = 'PIC_CANNOT_CONFIRM_OWN_CHALLENGE';
      err.status = 403;
      throw err;
    }

    // Verify confirming actor ID if pre-bound
    if (challenge.confirming_actor_id && consumingActorId && consumingActorId !== challenge.confirming_actor_id) {
      const err = new Error(`Actor mismatch: This challenge is bound to PIC ${challenge.confirming_actor_id}`);
      err.code = 'CONSUMER_ID_MISMATCH';
      err.status = 403;
      throw err;
    }
