/**
 * Songkick Source Adapter
 * Trusted Primary Event Source
 * 
 * Ingests concert dates and tour stops from Songkick.
...
class SongkickAdapter extends EventSourceAdapter {
  constructor(sourceId = 'src-songkick-jakarta', options = {}) {
  ...
  parse(raw) {
    ...
    const rawTitle = s.title || s.name || s.displayName || (Array.isArray(s.artists) ? s.artists.join(', ') : s.artist) || 'Concert';
    const normTitle = EventNormalizationService.normalizeTitle(rawTitle);
    const venueRaw = s.venue_name || s.venue || (s.venue && s.venue.displayName) || 'Venue TBA';
    ...
    const startDate = s.start_date || s.date || (s.start ? s.start.date : null);
    const endDate = s.end_date || (s.end ? s.end.date : null);
    const eventUrl = s.official_event_url || s.event_url || s.uri || s.url || s.source_url || (s.id ? `https://www.songkick.com/concerts/${s.id}` : 'https://www.songkick.com/');
    const sourceEventId = s.source_event_id || s.id || (s.uri ? s.uri.split('/').pop() : null);
    ...
    const organizer = s.organizer_name || s.organizer || s.promoter || 'Songkick Concert Listing';
