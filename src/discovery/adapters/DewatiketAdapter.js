/**
 * Dewatiket Source Adapter
 * Ticketing Platform for Central Java, DIY, and East Java
 * 
 * Ingests structured ticketing observations and decomposes factual claims.
 * Fail-closed passive mode if feed URL is unconfigured.
 */

const { EventSourceAdapter } = require('./EventSourceAdapter');

class DewatiketAdapter extends EventSourceAdapter {
  constructor(sourceId = 'src-dewatiket', options = {}) {
    super(sourceId, options);
    this.feedUrl = options.feedUrl || process.env.DEWATIKET_FEED_URL || null;
  ...
  async discover(query = {}) {
    ...
    if (!this.feedUrl) {
      return {
        ...
        reason: 'Dewatiket partner feed URL not configured; running in passive ingestion mode',
        events: []
      };
    }
    ...
    return this.fetchWithRetry(async () => {
      ...
      if (!res.ok) throw new Error(`Dewatiket feed returned HTTP ${res.status}`);
